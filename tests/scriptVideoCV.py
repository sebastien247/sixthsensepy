        ...
        for subdirname in dirnames:
            ...
            for filename in os.listdir(subject_path):
                try:
                    ...
                    # Redimensionner à taille donnée (si donnée )
                    if (sz is not None):
                        im = cv2.resize(im, sz)
...
#inizializzazione:
quanti = int(raw_input('combien de personnes? \n numero:'))
for i in range(quanti):
    nome = raw_input('utilisateur '+str(i+1)+' votre \n nome:')
    if not os.path.exists(pathdir+nome): os.makedirs(pathdir+nome)
    print ( 'vous êtes prêt pour prendre quelques photos ?? \n')
    print ( ' il faudra seulement 10 secondes\n clicker "S" centre')
    while (1):
        ret,frame = vc.read()
    ...
    while int(time.time()-start) <= 14:
        ...
        faces = face_cascade.detectMultiScale(gray, 1.2, 3)
        for (x,y,w,h) in faces:
            cv2.putText(frame,'load ...', (x,y), cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,250),3,1)
            count +=1
            cv2.rectangle(frame,(x,y),(x+w,y+h),(255,0,0),2)
...
while (1):
    ...
    for (x,y,w,h) in faces:
        ...
        if int(generic_classifier_output['distances']) <=  700:
            cv2.putText(img,'votre prenom : '+str(subject_dictionary[predicted_label]), (x,y), cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,250),3,1)
